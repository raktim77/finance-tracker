...
function resolveNativeChromeColor(pathname: string) {
  ...
  if (pathname === "/" || pathname === "/login" || pathname.startsWith("/oauth-finish")) return surface;
  if (pathname === "/dashboard") return elevated;
  return background;
}
...
function ScrollRestoration() {
  ...
  useEffect(() => {
    const restoreScroll = () => {
      ...
      if (hashId && scrollElementIntoView(hashId)) return;
...
function AppCore() {
  ...
  useEffect(() => {
    ...
    let smsListener: PluginListenerHandle | null = null;
    ...
    const init = async () => {
      smsListener = await SmsListener.addListener(
        "notificationClicked",
        (data: SmsNotification) => handleNotification(data)
      );
  ...
  return (
    <>
      ...
      <Routes>
        ...
        {isNativeAndroidApp() && (
          <Route path="/pending-review" element={<ProtectedRoute><DashboardLayout><PendingReview /></DashboardLayout></ProtectedRoute>} />
        )}
        <Route path="/transactions" element={<ProtectedRoute><DashboardLayout><Transactions /></DashboardLayout></ProtectedRoute>} />
        <Route path="/accounts/transactions/:account_id" element={<ProtectedRoute><DashboardLayout><Transactions /></DashboardLayout></ProtectedRoute>} />
        <Route path="/accounts" element={<ProtectedRoute><DashboardLayout><Accounts /></DashboardLayout></ProtectedRoute>} />
        <Route path="/budgets" element={<ProtectedRoute><DashboardLayout><Budgets /></DashboardLayout></ProtectedRoute>} />
        ...
        <Route path="/more" element={<ProtectedRoute><DashboardLayout><MorePage /></DashboardLayout></ProtectedRoute>} />
        <Route path="*" element={<Navigate to={isNativeAndroidApp() ? "/login" : "/"} replace />} />
      </Routes>
      <Analytics />
    </>
  );
